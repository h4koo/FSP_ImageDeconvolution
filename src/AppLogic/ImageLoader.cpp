...
namespace AppLogic
{
    ...
    bool ImageLoader::deleteFilter(const string filter_name){
        ...
        if(remove(full_path.c_str())!=0) return false;
    ...
    vector<VecImage> ImageLoader::loadImagesFromZip(string file_path){
        ...
        for (int i = 0; i < num_entries; ++i){
            ...
            if (zip_stat_index(z_arch, i, 0, &z_fstat) != 0) continue;
            
            if (!isSupportedImageFormat(z_fstat.name)) continue;
            ...
            if (ld_image.num_cols() == 0) continue;
    ...
    bool ImageLoader::createExportZip(string export_name, string filter_name){
        if (!existsNameFilter(filter_name)) return false;
        ...
        if (zipper == nullptr) {
            ...
            // printf("Failed to open output file " + output_filename + ": " + zip_error_strerror(&ziperror));
    ...
    bool ImageLoader::importFilterFromZip(string import_filename){
        ...
        {
            if (zip_stat_index(z_arch, i, 0, &z_fstat) != 0) continue;
            if (zfile_fdata_index != -1 && zfile_finfo_index != -1) break;
