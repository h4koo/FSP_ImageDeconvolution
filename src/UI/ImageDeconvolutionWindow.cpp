                                                                      ...
                                                                      ui(new Ui::ImageDeconvolutionWindow), label_loaded_image(new QLabel), label_filtered_image(new QLabel), is_filter_loaded (false)
{
    ...
    // this->loaded_images_model = new QStandardItemModel();
    // this->proxyModel.setSourceModel(&this->loaded_images_model);
    // ui->listWidget_loaded_images->setModel(&this->proxyModel);

    // connect(&this->future_watcher_load_filter, SIGNAL (finished()), this, SLOT (on_filterLoadingFinished()));
...
void ImageDeconvolutionWindow::loadImagesFromZip(const QString &folder_path){
    ...
    this->progress_dialog_filter = new QProgressDialog("Loading Images","Stop", 0,0, this);
...
void ImageDeconvolutionWindow::on_listWidget_loaded_images_currentRowChanged(int currentRow)
{
    ...
    if(this->is_filter_loaded) this->setFilteredImage(currentRow);
}
...
void ImageDeconvolutionWindow::on_comboBox_filter_select_currentIndexChanged(int index)
{
    ...
    statusBar()->showMessage("Loading filter matrix ...");
}
...
void ImageDeconvolutionWindow::on_filterAllImagesCanceled(){
    ...
    // this->bool_future_watcher.result();
    // this->bool_future_watcher.cancel();
    //show dialog with 

    // cancel image filtering
...
void ImageDeconvolutionWindow::on_pushButton_deletefilter_clicked()
{
    ...
    if(this->img_cleaning.deleteFilter(index)) ui->comboBox_filter_select->removeItem(index);
}
...
void ImageDeconvolutionWindow::on_pushButton_importfilter_clicked()
{
    ...
    uihelp::initializeImageFileDialog(dialog, QFileDialog::AcceptOpen, QFileDialog::FileMode::ExistingFile);
     if (dialog.exec() == QDialog::Accepted)
    {
...
void ImageDeconvolutionWindow::importFilter(const QString &import_filename){
    ...
    // return AppLogic::ImageLoader::importFilterFromZip(import_filename.toStdString());
}
...
void ImageDeconvolutionWindow::exportFilter(const QString &export_filename){
    ...
    // this->progress_dialog_calc_filter->setRange(0,0);
    this->progress_dialog_filter->exec();
    // return AppLogic::ImageLoader::createExportZip(export_filename.toStdString(),ui->comboBox_filter_select->currentText().toStdString());
}
...
void ImageDeconvolutionWindow::on_actionLoadImageFromZip_triggered(){
    ...
    void ImageDeconvolutionWindow::on_exportFilterCanceled(){
        AppLogic::ImageLoader::cancelOperation(true);
        this->bool_future_watcher.result(); // blocks until cancelation is finished
        statusBar()->showMessage(tr("Export filter canceled"), LONGER_MESSAGE_TIME);
        AppLogic::ImageLoader::cancelOperation(false);
    }
    ...
    void ImageDeconvolutionWindow::on_importFilterCanceled(){
        AppLogic::ImageLoader::cancelOperation(true);
        this->bool_future_watcher.result(); // blocks until cancelation is finished
        statusBar()->showMessage(tr("Importing filter canceled"), LONGER_MESSAGE_TIME);
        AppLogic::ImageLoader::cancelOperation(false);
